...
{
   template<>
   struct hash<bts::chain::pts_address> 
   {
       public:
         size_t operator()(const bts::chain::pts_address &a) const 
         {
            size_t s;
...
FC_REFLECT( bts::chain::pts_address, (addr) )

namespace fc 
{ 
   void to_variant( const bts::chain::pts_address& var,  fc::variant& vo );
   void from_variant( const fc::variant& var,  bts::chain::pts_address& vo );
}
