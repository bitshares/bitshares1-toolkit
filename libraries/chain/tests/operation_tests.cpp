...
BOOST_AUTO_TEST_CASE( create_asset )
{
   try {
      ...
      BOOST_CHECK(!db.push_transaction(trx));
      trx.operations.back().get<asset_create_operation>().symbol = "TEST" + fc::to_string(test_num++);
      ...
      BOOST_CHECK(!db.push_transaction(trx));
      trx.operations.back().get<asset_create_operation>().symbol = "TEST" + fc::to_string(test_num++);
      trx.operations.back().get<asset_create_operation>().symbol = "ABCDEFGHIJK";
